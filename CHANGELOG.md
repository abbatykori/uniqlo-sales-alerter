...
- **`ALERTER_SECRET` bootstrap** — new `secret.py` module. Loads from the `ALERTER_SECRET` env var when set; otherwise reads or auto-generates a 64-character hex secret persisted to `/app/data/.secret` (path overridable via `ALERTER_SECRET_PATH`). New file is written with mode `0600`. Wired into `AppState.secret` at lifespan startup; step 11 (HMAC action URLs) will consume it.
- **`check_history` writes** — every `run_sale_check` call now persists one row to the `check_history` table (success or failure), with `duration_ms`, `deals_scanned`, `deals_matched`, `deep_discounts` (count meeting `deep_discount_threshold`), and `error` on exception.
...
- **Deal heatmap (step 13)** — `run_sale_check` writes one `deal_observations` row per matched deal with `is_deep = (has_known_discount AND discount_percentage >= deep_discount_threshold)`. New `services/heatmap.py::aggregate()` groups by day-of-week × hour-of-day over a configurable lookback (default 90 days). `/ui/insights` renders a 7×24 grid with opacity scaled to `count / max_count` (clamped to [0.05, 1.0] so zero cells stay visible). Shows an "insufficient data" banner until 14 distinct days of observations accrue. Optional `?lookback=30|90|365` query param.
- **Invoice paste parser (step 14)** — `parsers/size.py` categorises raw size tokens into clothing (XXS..3XL), pants (22-40 inch), shoes (37-43 with optional `.5`), and kids (`<N>cm` or `<years> Years (<N>cm)` — canonicalised to `<N>cm`). `parsers/invoice_paste.py` walks pasted invoice text, matches the `<product_id>, <name>` then `<COLOUR>, <SIZE>` block pattern, dedups, and reports unparsed lines. Handles invisible Unicode (BOM, zero-width space) from PDF extraction. Currency-agnostic. New endpoints: `POST /api/v1/parsers/invoice` (JSON `text` in → categorised sizes out), `GET /ui/filters/paste` (textarea page), `POST /ui/filters/paste` (HTMX → chip-suggestion partial).
- **Upstream config migration (step 15)** — new `services/upstream_migration.py::ensure_upstream_migration` runs once per install (idempotent via `migrations_applied` row `upstream_v1`). Imports `config.filters.watched_variants` / `.ignored_products` / `.ignored_keywords` into their SQLite tables, parses `.seen_variants.json` into the `seen_variants` table (preserves the variant-key shape and discount precision), then moves the source files to `data/migrated/<UTC timestamp>/` (never deletes; preserves a recoverable backup per handoff working agreement). Pre-checks existing rows before insert so duplicate config entries and partial-failure retries are no-ops. Best-effort: corrupt `.seen_variants.json` logs a warning but doesn't abort the rest of the migration. Wired into lifespan + `reload_config` after `ensure_bridge_migration`.
- **UI polish — phase 16a (Tailwind + tokens + base shell)** — `design/tokens/tokens.json` is the W3C-Design-Tokens-Format source of truth for colour / radius / shadow (palette per Design Brief §3). `design/scripts/build-tokens.py` codegens `src/uniqlo_sales_alerter/ui/static/tokens.css`. The new `base.html` pulls Tailwind from CDN, registers token vars under Tailwind's theme palette (so `bg-canvas`, `text-text-primary` work), loads Noto Sans / Noto Serif / Noto Sans Mono, and provides a mobile-first layout: a bottom tab bar on mobile (Deals / Filters / Inbox / Insights / More) and a desktop sidebar with Settings + Help under a divider. FastAPI mounts `/static/*` for the generated CSS and future vendored Phosphor SVGs (step 16b).
