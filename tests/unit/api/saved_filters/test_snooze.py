"""Snooze + resume on /api/v1/filters/{id}/{snooze,resume} and /ui/filters/{id}."""
...
def test_ui_snooze_popover_renders_four_durations(client):
    fid = _create_filter(client)
    r = client.get(f"/ui/filters/{fid}/snooze")
    assert r.status_code == 200
...
def test_ui_snooze_post_returns_muted_row(client):
    fid = _create_filter(client)
    r = client.post(f"/ui/filters/{fid}/snooze", data={"duration": "1d"})
    assert r.status_code == 200
...
def test_ui_snooze_forever_renders_indefinite_label(client):
    fid = _create_filter(client)
    r = client.post(f"/ui/filters/{fid}/snooze", data={"duration": "forever"})
    assert r.status_code == 200
...
def test_ui_resume_clears_snooze_state(client):
    fid = _create_filter(client)
    client.post(f"/ui/filters/{fid}/snooze", data={"duration": "7d"})
    r = client.post(f"/ui/filters/{fid}/resume")
    assert r.status_code == 200
...
def test_ui_unsnoozed_row_shows_snooze_button(client):
    ...
    assert "hx-get=\"/ui/filters/" in r.text
...
def test_snooze_then_resume_is_idempotent(client):
    ...
    r = client.post(f"/ui/filters/{fid}/snooze", data={"duration": "30d"})
    assert r.status_code == 200
    # Resume twice — second call still 200 (no error even though already resumed)
    assert client.post(f"/ui/filters/{fid}/resume").status_code == 200
    assert client.post(f"/ui/filters/{fid}/resume").status_code == 200
